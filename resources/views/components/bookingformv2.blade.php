						...
						<div class="col-md-6 ftco-animate p-md-1 img img-2 d-flex justify-content-center">
							...
							<div id = "tallydiv">
								<fieldset id = "visitorset" name = "tvisitor">
									<div class="row" x-data="{ bookings: {{$ubookings}}, selectedBooking: null, }">
										<div class="col-sm-4">
								...
								<tbody>
									...
									<tr>
										...
										<td rowspan="6" id = "leadtd">
											...
											{{ $users->firstWhere('id', Auth::id())->name }}  {{ $users->firstWhere('id', Auth::id())->lname }}, {{ date_diff(date_create($users->firstWhere('id', Auth::id())->bdate), date_create(date('Y-m-d')))->y }}, @if ($users->firstWhere('id', Auth::id())->gender === 'Male')<b>M</b> @elseif ($users->firstWhere('id', Auth::id())->gender === 'Female')<b>F</b>  @endif
											<br>{{ $users->firstWhere('id', Auth::id())->email }}<br>
											{{ $users->firstWhere('id', Auth::id())->contactno }}<br><br>
											<span style="color:black;"><b>COUNTRY</b></span><br>
											@if($users->firstWhere('id', Auth::id())->country === 'INTERNATIONAL')<img src ="/images/nation/un.webp" style = "width: 2rem; height: 1rem; display: inline-flex;" alt="INT" />
											@elseif($users->firstWhere('id', Auth::id())->country === 'PHILIPPINES')<img src ="/images/nation/ph.webp" style = "width: 2rem; height: 1rem; display: inline-flex;" alt="PH" />  
											@elseif($users->firstWhere('id', Auth::id())->country === 'MAUBAN')<img src ="/images/nation/mn.webp" style = "width: 2rem; height: 1.5rem; display: inline-flex;" alt="MN" />  
											@endif
											&nbsp;{{ $users->firstWhere('id', Auth::id())->region }}
										</td>
			...
			<div class="container">
				<div class="row no-gutters slider-text align-items-center justify-content-center">
					<div class="col-md-12 ftco-animate text-center">
						...
						<h4 id="submittext">
							Thank you for your submission! We're pleased to confirm that your booking has been successfully received. You can easily modify your booking at any time through your <a href="{{ url('dashboard')}}" id="dashbtn">dashboard</a> if needed.
							<br><br>
						...
						<div class="row">
							<div class="col-6">
								<button type="button" class="sav-btn" @click="document.location.href = '{{ url('/') }}';">Home</button>
							</div>
							<div class="col-6">
								<button type="button" class="sav-btn" @click="document.location.href = '{{ Auth::user()->role_id == 979 ? route('admin.dashboard') : (Auth::user()->role_id == 797 ? route('owner.dashboard') : route('user.dashboard'))}}';">Dashboard</button>
							</div>
						</div>
					</div>
				</div>
